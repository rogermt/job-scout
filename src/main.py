...
from tracking.database import Job
...
@click.option("--location", "-l", required=False, help="Location filter (e.g., 'UK Remote')")
@click.option("--max-pages", "-p", type=int, default=3, help="Max pages to scrape per platform")
@click.pass_context
def search(ctx: click.Context, query: str, location: Optional[str], max_pages: int) -> None:
    ...
    for platform_name, platform_config in enabled_platforms.items():
        ...
        if not scraper:
            console.print(f"  [red]✗[/red] Scraper not available")
            continue
