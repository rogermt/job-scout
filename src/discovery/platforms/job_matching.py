...
class JobMatcher:
    ...
    def match_job(self, job_data: Dict[str, Any]) -> Tuple[bool, float, Dict[str, Any]]:
        ...
        matches = score >= self.preferences.get("min_score_threshold", 40.0)
        
        if matches:
            logger.debug("Job matched preferences", extra={
    ...
    def _has_excluded_keywords(self, job_data: Dict[str, Any]) -> bool:
        ...
        for pattern in self.exclude_patterns:
            if pattern.search(text_to_check):
                logger.debug("Job excluded by keyword filter", extra={
                    "job_id": job_data.get("id"),
    ...
    def _score_location(self, job_data: Dict[str, Any]) -> float:
        ...
        uk_indicators = ["uk", "united kingdom", "britain", "england", "scotland", "wales", "northern ireland", "london"]
    ...
    def _score_salary(self, job_data: Dict[str, Any]) -> float:
        ...
        target_min = self.preferences.salary.get("min_gbp", 0)
    ...
    def filter_jobs(self, jobs: List[Dict[str, Any]]) -> List[Tuple[Dict[str, Any], float, Dict[str, Any]]]:
        ...
        for job in jobs:
            try:
                ...
                logger.error("Error matching job", extra={
                    "job_id": job.get("id"),
                    "error": str(e)
                }, exc_info=True)
                continue
        ...
        logger.info("Job matching complete", extra={
            "total_jobs": len(jobs),
    ...
    def get_match_explanation(self, job_data: Dict[str, Any], score: float, reasons: Dict[str, Any]) -> str:
