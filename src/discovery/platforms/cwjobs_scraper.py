...
class CwjobsScraper(BaseScraper):
    ...
    def extract_job_listings(self, soup: BeautifulSoup) -> list[Tag]:
        return soup.select("div.job-card, article.job-job")

    def parse_job_listing(self, element: Tag) -> Optional[dict[str, Any]]:
        job_id = element.get("data-job-id") or element.get("id", "")

        title_elem = element.select_one("h2.job-title a, h3 a")
        title = title_elem.get_text(strip=True) if title_elem else "Unknown"

        company_elem = element.select_one("span.company-name, .company")
        company = company_elem.get_text(strip=True) if company_elem else ""

        location_elem = element.select_one("span.location, .job-location")
        location_text = location_elem.get_text(strip=True) if location_elem else ""
        location = {"original": location_text}

        salary_elem = element.select_one("span.salary, .salary")
        salary_text = salary_elem.get_text(strip=True) if salary_elem else ""
        salary = self._parse_salary(salary_text)

        url_elem = element.select_one("h2.job-title a, a.job-title")
        ...
        description_elem = element.select_one("p.summary, .job-summary")
