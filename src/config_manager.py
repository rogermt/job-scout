...
class SalaryPreferences(BaseModel):
    ...
    currency_preference: str = Field(default="GBP", description="Preferred currency for display")
...
class JobPreferences(BaseModel):
    ...
    keywords: list[str] = Field(default_factory=list, description="Required keywords in job description")
...
class PlatformConfig(BaseModel):
    ...
    region: Optional[str] = Field(default=None, description="Region filter for the platform")
...
class AIConfig(BaseModel):
    ...
    cover_letter_generation: bool = Field(default=True, description="Enable cover letter generation")
...
class DatabaseConfig(BaseModel):
    ...
    path: Path = Field(
        default=Path("output/job_scout.db"),
...
class OutputConfig(BaseModel):
    ...
    tailored_cv_dir: Path = Field(
        default=Path("output/tailored_cvs"),
        description="Directory for tailored CVs"
    )
    cover_letter_dir: Path = Field(
        default=Path("output/cover_letters"),
        description="Directory for cover letters"
    )
    analytics_dir: Path = Field(
        default=Path("output/analytics"),
        description="Directory for analytics reports"
    )
    log_file: Path = Field(
        default=Path("logs/job_scout.log"),
...
class SchedulingConfig(BaseModel):
    ...
    search_frequency_hours: int = Field(
        default=6,
        description="How often to run job searches (hours)"
    )
...
class PersonalDetails(BaseModel):
    ...
    website: Optional[HttpUrl] = Field(default=None, description="Personal website/portfolio")
