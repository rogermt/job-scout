...
class JobMatchingTester:
    ...
    def test_uk_location_detection(self):
        ...
        test_jobs = [
            ...
            },
            {
                ...
                "location": {
                    "original": "Remote, United Kingdom",
                    "city": "Remote",
                    "country": "UK",
                },
                "salary": {
        ...
        for job in test_jobs:
            ...
            print(
                f"     Score: {score:.1f}/100 | UK: {is_uk} | Remote: {job['remote_policy']}"
            )
    ...
    def test_salary_scoring_gbp(self):
        ...
        for job in test_jobs:
            ...
            print(
                f"  {'✅' if passed else '❌'} {job['id']}: {salary['min']:,} {salary['currency']} = {salary_gbp:,.0f} GBP"
            )
    ...
    def test_remote_bonuses(self):
        ...
        for job in test_jobs:
            ...
            print(
                f"  {'✅' if passed else '❌'} {job['id']}: {remote_policy} (score: {remote_score:.1f})"
            )
    ...
    def test_exclusion_filters(self):
        ...
        for job in test_jobs:
            ...
            print(
                f"  {'✅' if passed else '❌'} {job['id']}: {'EXCLUDED' if not matches else 'MATCHED'} (has_exclusion: {has_exclusion})"
            )
    ...
    def test_match_scores(self):
        ...
        perfect_matches, perfect_score, perfect_reasons = self.matcher.match_job(
            perfect_job
        )
        mediocre_matches, mediocre_score, mediocre_reasons = self.matcher.match_job(
            mediocre_job
        )
        ...
        print(
            f"  {'✅' if mediocre_ok else '❌'} Mediocre match: {mediocre_score:.1f}/100"
        )
    ...
    def test_various_job_platforms(self):
        ...
        for job in platform_jobs:
            ...
            is_recruiter = (
                "recruit" in job["company"].lower() or "recruit" in job["title"].lower()
            )
